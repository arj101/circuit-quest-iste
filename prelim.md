...
8. Given two 4 bit numbers $a_0, ... a_3$ and $b_0, ... b_3$, output a 4 bit number $c_0, ... c_3$ containing the bitwise AND of the two input numbers.
9. The given 7-segment display has 7 binary inputs, (name of each input given within the segment), the segments light up (green) when its input is 1, and the segment is dark when its input is 0. Given an input $a$, output the corresponding patterns given in the image below.![](./q9.png)
